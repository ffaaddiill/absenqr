        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                if ($post_image['image']['tmp_name']) {
                    ...
                    try {
                        //Upload blob
                        $picture_db = $this->azure->UploadFileToStorage($post_image['image'],AZURE_FOLDER_UPLOADS,'admin/'.$filename);
                        $this->Admin_model->UpdateRecord($id,array('image'=>$picture_db));
                        // resize image
                        $thumb_prefix = 'admin/thumb/';
                        $source_file = AZURE_BLOB_URLPREFIX.AZURE_FOLDER_UPLOADS.'/'.$picture_db;
                        $this->azure->ResizeUploadImage($source_file,UPLOAD_DIR.'admin/',AZURE_FOLDER_UPLOADS,$thumb_prefix,$filename,IMG_THUMB_WIDTH,IMG_THUMB_HEIGHT);
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                if ($post_image['image']['tmp_name']) {
                    $filename = 'adm_'.url_title($post['name'],'_',true).md5plus($id);
                    /**
                     * disabled coz now we're using azure storage
