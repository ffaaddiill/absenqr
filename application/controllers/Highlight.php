...
class Highlight extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['export_excel_url']=site_url('highlight/export_excel_highlight');
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Highlight_model->CountAllHighlight();
            $count_filtered_records = $this->Highlight_model->CountAllHighlight($param);
            $records = $this->Highlight_model->GetAllHighlightData($param);
            $return = array();
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                // insert data
                $id = $this->Highlight_model->InsertRecord($post);
                ...
                if ($post_image['thumbnail_image']['tmp_name']) {
                    ...
                    try {
                        //Upload blob
                        $picture_db = $this->azure->UploadFileToStorage($post_image['thumbnail_image'],AZURE_FOLDER_UPLOADS,'highlight/'.$filename);
                        $this->Highlight_model->UpdateRecord($id,array('thumbnail_image'=>$picture_db));
                ...
                if ($post_image['primary_image']['tmp_name']) {
                    ...
                    try {
                        //Upload blob
                        $picture_db = $this->azure->UploadFileToStorage($post_image['primary_image'],AZURE_FOLDER_UPLOADS,'highlight/'.$filename);
                        $this->Highlight_model->UpdateRecord($id,array('primary_image'=>$picture_db));
                        // resize image
                        $thumb_prefix = 'highlight/thumb/';
                        $source_file = AZURE_BLOB_URLPREFIX.AZURE_FOLDER_UPLOADS.'/'.$picture_db;
                        $this->azure->ResizeUploadImage($source_file,UPLOAD_DIR.'highlight/',AZURE_FOLDER_UPLOADS,$thumb_prefix,$filename,IMG_THUMB_WIDTH,IMG_THUMB_HEIGHT);
                    } catch (ServiceException $e) {
                        $code = $e->getCode();
                        $error_message = $e->getMessage();
                        echo $code . ": " . $error_message . "<br />";
                ...
                $data_log = array(
                    ...
                    'action' => 'Highlight',
                    'desc' => 'Add Highlight; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Highlight_model->GetHighlight($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                // update data
                $this->Highlight_model->UpdateRecord($id,$post);
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Highlight',
                    'desc' => 'Edit Highlight; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Highlight_model->DeleteRecord($id);
    ...
    public function delete_picture() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                $detail = $this->Highlight_model->GetHighlight($post['id']);
                ...
                if ($detail && ($detail[$type.'_image'] != '')) {
                    ...
                    if ($this->azure->DeleteBlob(AZURE_FOLDER_UPLOADS,$detail[$type.'_image'] == TRUE)) {
                        ...
                        $this->Highlight_model->UpdateRecord($post['id'],$data_update);
                        ...
                        $data_log = array(
                            ...
                            'id_group' => id_auth_group(),
                            'action' => 'Highlight',
                            'desc' => 'Delete '.ucfirst($type).' Picture Highlight; ID: '.$id.';',
                        );
                        insert_to_log($data_log);
                        // end insert to log
                    } else {
     ...
     **/
    function export_excel_highlight() {
        $this->layout = 'none';
        $list=array();
        $post=$this->input->post();
        $record = $this->Highlight_model->GetAllHighlightData();
        $name_file=url_title('highlight',true);
        ...
        $this->parser->parse(TEMPLATE_DIR.'/highlight/excel.html', $this->data);
        export_to($ouput_file_name);
    }
}
/* End of file Highlight.php */
/* Location: ./application/controllers/Highlight.php */