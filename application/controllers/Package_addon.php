 ...
 * 
 */
class Package_addon extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Package_addon_model');
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Package_addon_model->CountAllRecord();
            $count_filtered_records = $this->Package_addon_model->CountAllRecord($param);
            $records = $this->Package_addon_model->GetAllRecordData($param);
            $return = array();
            ...
            foreach ($records as $row => $record) {
                ...
                ';
                $return['data'][$row]['addon_name'] = $record['addon_name'];
                $return['data'][$row]['position'] = $record['position'];
                $return['data'][$row]['id_status'] = ($record['id_status'] == 1) ? 'Active' : 'Not Active';
                $return['data'][$row]['price_addon'] = 'Rp. '.number_format($record['price_addon'],0);
    ...
    public function add() {
        ...
        $this->data['channels'] = $this->Package_addon_model->GetChannels();
        if ($this->input->post()) {
            $post = $this->input->post();
            if ($this->validateForm()) {
                ...
                //print_r($post);exit();
                // insert data
                $id = $this->Package_addon_model->InsertRecord($post);
                
                // input channel
                $channel_array = array();
                // purge data before new insert
                $this->Package_addon_model->DeletePackageChannel($id);
                ...
                if ($channel_array) {
                    $this->Package_addon_model->InsertPackageChannel($channel_array);
                ...
                $data_log = array(
                    ...
                    'action' => 'Package_addon',
                    'desc' => 'Add Package Addon; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Package_addon_model->GetRecord($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                // update data
                $this->Package_addon_model->UpdateRecord($id,$post);
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Package_addon_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
