 ...
 * 
 */
class Package_decoder extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Package_decoder_model');
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Package_decoder_model->CountAllRecord();
            $count_filtered_records = $this->Package_decoder_model->CountAllRecord($param);
            $records = $this->Package_decoder_model->GetAllRecordData($param);
            $return = array();
            ...
            foreach ($records as $row => $record) {
                ...
                ';
                $return['data'][$row]['decoder_name'] = $record['decoder_name'];
                $return['data'][$row]['id_status'] = ($record['id_status'] == 1) ? 'Active' : 'Not Active';
                $return['data'][$row]['price_decoder'] = 'Rp. '.number_format($record['price_decoder'],0);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $post['position'] = (isset($post['position'])) ? $post['position'] : 0;
                
                //print_r($post);exit();
                // insert data
                $id = $this->Package_decoder_model->InsertRecord($post);
                ...
                $data_log = array(
                    ...
                    'action' => 'Package_decoder',
                    'desc' => 'Add Package Decoder; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Package_decoder_model->GetRecord($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                // update data
                $this->Package_decoder_model->UpdateRecord($id,$post);
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Package',
                    'desc' => 'Edit Package Decoder; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Package_decoder_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Package Decorder',
                                'desc' => 'Delete Package Decoder; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
