...
class Regional extends CI_Controller {
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Regional_model->CountAllRecord();
            $count_filtered_records = $this->Regional_model->CountAllRecord($param);
            $records = $this->Regional_model->GetAllRecordData($param);
            $return = array();
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                // insert data
                $id = $this->Regional_model->InsertRecord($post);
    ...
    public function edit($id=0) {
        ...
        $record = $this->Regional_model->GetRecord($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $this->Regional_model->UpdateRecord($id,$post);
                // insert to log
                $data_log = array(
                    'id_user' => id_auth_user(),
                    'id_group' => id_auth_group(),
                    'action' => 'Regional',
                    'desc' => 'Edit Regional; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
            ...
            if ($post['ids'] != '') {
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        $record = $this->Regional_model->GetRecord($id);
                        if ($record) {
                            $this->Regional_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Regional',
                                'desc' => 'Delete Regional; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
