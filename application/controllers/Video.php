...
class Video extends CI_Controller {
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Video_model->CountAllVideo();
            $count_filtered_records = $this->Video_model->CountAllVideo($param);
            $records = $this->Video_model->GetAllVideoData($param);
            $return = array();
    ...
    public function add() {
        ...
        //     );
        // $this->data['categorys'] = $category_video;
        // echo '<pre>';
        // print_r(get_sites());
        // die();
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                // insert data
                $id = $this->Video_model->InsertRecord($post);
                ...
                if($id){
                    ...
                    if (count($sites)>0) {
                        $this->Video_model->InsertSitesBatch($sites);
                    }
                }
                ...
                if ($post_image['thumbnail_image']['tmp_name']) {
                    ...
                    try {
                        //Upload blob
                        $picture_db = $this->azure->UploadFileToStorage($post_image['thumbnail_image'],AZURE_FOLDER_UPLOADS,'video/'.$filename);
                        $this->Video_model->UpdateRecord($id,array('thumbnail_image'=>$picture_db));
                        // resize image
                        //$thumb_prefix = 'video/thumb/';
                        //$source_file = AZURE_BLOB_URLPREFIX.AZURE_FOLDER_UPLOADS.'/'.$picture_db;
                        //$this->azure->ResizeUploadImage($source_file,UPLOAD_DIR.'video/',AZURE_FOLDER_UPLOADS,$thumb_prefix,$filename,IMG_THUMB_WIDTH,IMG_THUMB_HEIGHT);
                ...
                if ($post_image['primary_image']['tmp_name']) {
                    ...
                    try {
                        //Upload blob
                        $picture_db = $this->azure->UploadFileToStorage($post_image['primary_image'],AZURE_FOLDER_UPLOADS,'video/'.$filename);
                        $this->Video_model->UpdateRecord($id,array('primary_image'=>$picture_db));
                        // resize image
                        $thumb_prefix = 'video/thumb/';
                        $source_file = AZURE_BLOB_URLPREFIX.AZURE_FOLDER_UPLOADS.'/'.$picture_db;
                        $this->azure->ResizeUploadImage($source_file,UPLOAD_DIR.'video/',AZURE_FOLDER_UPLOADS,$thumb_prefix,$filename,IMG_THUMB_WIDTH,IMG_THUMB_HEIGHT);
                ...
                $data_log = array(
                    ...
                    'action' => 'Video',
                    'desc' => 'Add Video; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Video_model->GetVideo($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        $this->data['cancel_url'] = site_url($this->class_path_name);
        $this->data['sites'] = get_sites();
        $this->data['list_site'] = array_column($this->Video_model->GetSitesById($id),'id_site');
        $category_video = array(
            array(
                'id_category'=>1,
                'name'=>'Promo'
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                // update data
                $this->Video_model->UpdateRecord($id,$post);
                ...
                if (count($sites)>0) {
                    $this->Video_model->DeleteSite($id);
                    $this->Video_model->InsertSitesBatch($sites);
                }
                $post_image = $_FILES;
                if ($post_image['thumbnail_image']['tmp_name']) {
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Video',
                    'desc' => 'Edit Video; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
            ...
            if ($post['ids'] != '') {
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        $record = $this->Video_model->GetVideo($id);
                        if ($record) {
                            /*if ($record['thumbnail_image'] != '' && file_exists(UPLOAD_DIR.'video/'.$record['thumbnail_image'])) {
                                unlink(UPLOAD_DIR.'video/'.$record['thumbnail_image']);
                            }
                            if ($record['primary_image'] != '' && file_exists(UPLOAD_DIR.'video/'.$record['primary_image'])) {
                                unlink(UPLOAD_DIR.'video/'.$record['primary_image']);
                            }*/
                            $this->Video_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Video',
                                'desc' => 'Delete Video; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
                ...
                if ($detail && ($detail[$type.'_image'] != '')) {
                    ...
                    if ($this->azure->DeleteBlob(AZURE_FOLDER_UPLOADS,$detail[$type.'_image'] == TRUE)) {
                        ...
                        $data_log = array(
                            ...
                            'id_group' => id_auth_group(),
                            'action' => 'Video',
                            'desc' => 'Delete '.ucfirst($type).' Picture Video; ID: '.$id.';',
                        );
                        insert_to_log($data_log);
                        // end insert to log
                    } else {
    ...
    private function validateForm($id=0) {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if(!$id){
                if (!check_exist_uri('video',$post['uri_path'])) {
