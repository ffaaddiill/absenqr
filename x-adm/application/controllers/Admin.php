        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                if ($post_image['image']['tmp_name']) {                    
                    ...
                    $this->Admin_model->UpdateRecord($id,array('image'=>$this->class_path_name.'/'.$picture_db,'picture_file_name'=>$picture_db));
                    ...
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            if ($id == id_auth_user()) {
                                ...
                                if (is_superadmin()) {
                                    if ($record['image'] != '' && file_exists(UPLOAD_DIR.$record['image'])) {
                                        unlink(UPLOAD_DIR.$record['image']);
                                    }
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                ...
                if ($detail && $detail['image'] != '') {
                    $id = $post['id'];
                    unlink(UPLOAD_DIR.$detail['image']);
                    $this->Admin_model->delete_picture($id);
                    $json['success'] = true;
                } else {
