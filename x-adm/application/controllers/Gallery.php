...
class Gallery extends CI_Controller {
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Gallery_model->CountAllGallery();
            $count_filtered_records = $this->Gallery_model->CountAllGallery($param);
            $records = $this->Gallery_model->GetAllGalleryData($param);
            $return = array();
    ...
    public function add() {
        ...
        $this->data['list_product'] = $this->Gallery_model->getActiveProduct();
        $this->data['product_slideshow'] = $this->Gallery_model->getGalleryProduct( $this->uri->segment(3) );*/
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $post['position'] = ($post['position'] == '') ? $this->Gallery_model->GetMaxPosition() : $post['position'];
                
                // insert data
                $id = $this->Gallery_model->InsertRecord($post);
               ...
               /* if($id){
                    ...
                    if (count($sites)>0) {
                        $this->Gallery_model->InsertSitesBatch($sites);
                    }
                }*/
                ...
                if ($post_image['primary_image']['tmp_name']) {
                    ...
                    $this->Gallery_model->UpdateRecord($id,array('primary_image'=>$this->class_path_name.'/'.$picture_db,'picture_file_name'=>$picture_db));
                }
                // insert to log
                $data_log = array(
                    ...
                    'action' => 'Gallery',
                    'desc' => 'Add Gallery; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Gallery_model->GetGallery($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        /*
        $this->data['sites'] = get_sites();
        $this->data['list_site'] = array_column($this->Gallery_model->GetSitesById($id),'id_site');
       ...
       $this->data['list_product'] = $this->Gallery_model->getActiveProduct();
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                echo '</pre>';
                die();*/
                $this->Gallery_model->UpdateRecord($id,$post);
                ...
                /*if (count($sites)>0) {
                    $this->Gallery_model->DeleteSite($id);
                    $this->Gallery_model->InsertSitesBatch($sites);
                }*/
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Gallery',
                    'desc' => 'Edit Gallery; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Gallery_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Gallery',
                                'desc' => 'Delete Gallery; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                ...
                if ($record && $record['primary_image'] != '') {
                    unlink(UPLOAD_DIR.$record['primary_image']);
                    $this->Gallery_model->delete_picture($id);
                    $json['success'] = true;
                } else {
