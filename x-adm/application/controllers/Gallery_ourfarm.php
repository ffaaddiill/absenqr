...
/**
 * Gallery_ourfarm Class
 * @author ivan lubis <ivan.z.lubis@gmail.com>
 * @version 3.0
 * @category Controller
 * @desc Gallery_ourfarm Controller
 * 
 */
class Gallery_ourfarm extends CI_Controller {
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Gallery_ourfarm_model->CountAllGallery_ourfarm();
            $count_filtered_records = $this->Gallery_ourfarm_model->CountAllGallery_ourfarm($param);
            $records = $this->Gallery_ourfarm_model->GetAllGallery_ourfarmData($param);
            $return = array();
    ...
    public function add() {
        ...
        $this->data['list_product'] = $this->Gallery_ourfarm_model->getActiveProduct();
        $this->data['product_slideshow'] = $this->Gallery_ourfarm_model->getGallery_ourfarmProduct( $this->uri->segment(3) );*/
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $post['position'] = ($post['position'] == '') ? $this->Gallery_ourfarm_model->GetMaxPosition() : $post['position'];
                
                // insert data
                $id = $this->Gallery_ourfarm_model->InsertRecord($post);
               ...
               /* if($id){
                    ...
                    if (count($sites)>0) {
                        $this->Gallery_ourfarm_model->InsertSitesBatch($sites);
                    }
                }*/
                ...
                if ($post_image['primary_image']['tmp_name']) {
                    ...
                    $this->Gallery_ourfarm_model->UpdateRecord($id,array('primary_image'=>$this->class_path_name.'/'.$picture_db,'picture_file_name'=>$picture_db));
                }
                // insert to log
                $data_log = array(
                    'id_user' => id_auth_user(),
                    'id_group' => id_auth_group(),
                    'action' => 'Gallery_ourfarm',
                    'desc' => 'Add Gallery_ourfarm; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Gallery_ourfarm_model->GetGallery_ourfarm($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        /*
        $this->data['sites'] = get_sites();
        $this->data['list_site'] = array_column($this->Gallery_ourfarm_model->GetSitesById($id),'id_site');
       ...
       $this->data['list_product'] = $this->Gallery_ourfarm_model->getActiveProduct();
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                echo '</pre>';
                die();*/
                $this->Gallery_ourfarm_model->UpdateRecord($id,$post);
                ...
                /*if (count($sites)>0) {
                    $this->Gallery_ourfarm_model->DeleteSite($id);
                    $this->Gallery_ourfarm_model->InsertSitesBatch($sites);
                }*/
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Gallery_ourfarm',
                    'desc' => 'Edit Gallery_ourfarm; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Gallery_ourfarm_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Gallery_ourfarm',
                                'desc' => 'Delete Gallery_ourfarm; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                ...
                if ($record && $record['primary_image'] != '') {
                    unlink(UPLOAD_DIR.$record['primary_image']);
                    $this->Gallery_ourfarm_model->delete_picture($id);
                    $json['success'] = true;
                } else {
...
/* End of file Gallery_ourfarm.php */
/* Location: ./application/controllers/Gallery_ourfarm.php */