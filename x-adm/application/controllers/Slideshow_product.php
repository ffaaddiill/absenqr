...
/**
 * Slideshow_product Class
 * @author ivan lubis <ivan.z.lubis@gmail.com>
 * @version 3.0
 * @category Controller
 * @desc Slideshow_product Controller
 * 
 */
class Slideshow_product extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Slideshow_product_model');
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Slideshow_product_model->CountAllSlideshow_product();
            $count_filtered_records = $this->Slideshow_product_model->CountAllSlideshow_product($param);
            $records = $this->Slideshow_product_model->GetAllSlideshow_productData($param);
            $return = array();
    ...
    public function add() {
        ...
        $this->data['list_product'] = $this->Slideshow_product_model->getActiveProduct();
        $this->data['product_slideshow'] = $this->Slideshow_product_model->getSlideshow_productProduct( $this->uri->segment(3) );*/
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $post['position'] = ($post['position'] == '') ? $this->Slideshow_product_model->GetMaxPosition() : $post['position'];
                
                // insert data
                $id = $this->Slideshow_product_model->InsertRecord($post);
               ...
               /* if($id){
                    ...
                    if (count($sites)>0) {
                        $this->Slideshow_product_model->InsertSitesBatch($sites);
                    }
                }*/
                ...
                if ($post_image['primary_image']['tmp_name']) {
                    ...
                    $this->Slideshow_product_model->UpdateRecord($id,array('primary_image'=>$this->class_path_name.'/'.$picture_db,'picture_file_name'=>$picture_db));
                }
                // insert to log
                $data_log = array(
                    'id_user' => id_auth_user(),
                    'id_group' => id_auth_group(),
                    'action' => 'Slideshow_product',
                    'desc' => 'Add Slideshow_product; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Slideshow_product_model->GetSlideshow_product($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        /*
        $this->data['sites'] = get_sites();
        $this->data['list_site'] = array_column($this->Slideshow_product_model->GetSitesById($id),'id_site');
       ...
       $this->data['list_product'] = $this->Slideshow_product_model->getActiveProduct();
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                echo '</pre>';
                die();*/
                $this->Slideshow_product_model->UpdateRecord($id,$post);
                ...
                /*if (count($sites)>0) {
                    $this->Slideshow_product_model->DeleteSite($id);
                    $this->Slideshow_product_model->InsertSitesBatch($sites);
                }*/
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'Slideshow_product',
                    'desc' => 'Edit Slideshow_product; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                ...
                if (count($array_id)>0) {
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            ...
                            $this->Slideshow_product_model->DeleteRecord($id);
                            // insert to log
                            $data_log = array(
                                'id_user' => id_auth_user(),
                                'id_group' => id_auth_group(),
                                'action' => 'Delete Slideshow_product',
                                'desc' => 'Delete Slideshow_product; ID: '.$id.';',
                            );
                            insert_to_log($data_log);
                            // end insert to log
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                ...
                if ($record && $record['primary_image'] != '') {
                    unlink(UPLOAD_DIR.$record['primary_image']);
                    $this->Slideshow_product_model->delete_picture($id);
                    $json['success'] = true;
                } else {
...
/* End of file Slideshow_product.php */
/* Location: ./application/controllers/Slideshow_product.php */