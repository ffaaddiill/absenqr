...
class Vendor extends CI_Controller {
    ...
    public function list_data() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records = $this->Vendor_model->CountAllVendor();
            $count_filtered_records = $this->Vendor_model->CountAllVendor($param);
            $records = $this->Vendor_model->GetAllVendorData($param);
            $return = array();
            ...
            foreach ($records as $row => $record) {
                ...
                $return['data'][$row]['npwp'] = $record['npwp'];
     ...
     */
    public function add() {
        $this->data['categorys'] = $this->Vendor_model->GetCategoryVendor();
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                // inser data
                $id = $this->Vendor_model->InsertRecord($post);
                ...
                $data_log = array(
                    ...
                    'action' => 'User Vendor',
                    'desc' => 'Add  Vendor; ID: '.$id.'; Data: '.json_encode($post),
                );
    ...
    public function edit($id=0) {
        ...
        $record = $this->Vendor_model->GetVendor($id);
        if (!$record) {
            redirect($this->class_path_name);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                $this->Vendor_model->UpdateRecord($id,$post);
                ...
                $data_log = array(
                    ...
                    'id_group' => id_auth_group(),
                    'action' => 'User Vendor',
                    'desc' => 'Edit Vendor; ID: '.$id.'; Data: '.json_encode($post),
                );
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.','success'));
                    ...
                    foreach ($array_id as $row => $id) {
                        ...
                        if ($record) {
                            if (is_superadmin()) {                              
                                    ...
                                    $this->Vendor_model->DeleteRecord($id);
                                    ...
                                    $data_log = array(
                                        ...
                                        'id_group' => id_auth_group(),
                                        'action' => 'User Vendor',
                                        'desc' => 'Delete User Vendor; ID: '.$id.';',
                                    );
                                    insert_to_log($data_log);
                            } else {
                                $json['error'] = alert_box('You don\'t have permission to delete this record(s). Please contact the Vendoristrator.','danger');
                                break;
                            }
                        } else {
    ...
    public function delete_picture() {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                $detail = $this->Vendor_model->GetVendor($post['id']);
                if ($detail && $detail['image'] != '') {
                    ...
                    try {
                        ...
                        $this->Vendor_model->UpdateRecord($post['id'],$data_update);
                        ...
                        $data_log = array(
                            ...
                            'id_group' => id_auth_group(),
                            'action' => 'User Vendor',
                            'desc' => 'Delete Picture User Vendor; ID: '.$id.';',
                        );
                        insert_to_log($data_log);
                        // end insert to log
                    }
    ...
    public function check_email_exists($string,$id=0) {
        if (!$this->Vendor_model->checkExistsEmail($string, $id)) {
    ...
    public function check_username_exists($string,$id=0) {
        if (!$this->Vendor_model->checkExistsUsername($string, $id)) {
