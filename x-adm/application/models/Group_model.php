    ...
    function PrintAuthMenu($menus=array(),$prefix='') {
        ...
        if ($menus) {
            foreach ($menus as $row => $menu) {
                $return .= ($menu['parent_auth_menu'] != 0)?'<div class="checkbox site-auth-root">':'<div class="checkbox">';
                if ($menu['parent_auth_menu'] != 0) {
                    $return .= $prefix;
                    $return .= '<i class="fa fa-level-up" style="transform: rotateZ(90deg);" aria-hidden="true"></i>&nbsp;&nbsp;';
                } else {
                    $prefix = '';
                }
                $checked = '';
