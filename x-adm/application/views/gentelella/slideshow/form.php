            ...
            <div class="x_content">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        <div class="form-group">
                            ...
                            <div class="checkbox">
                                <label class="mb-0">
                                    <input type="checkbox" class="flat" value="1" name="is_video" id="is_video" <?= (isset($post['is_video']) && $post['is_video']) ? 'checked="checked"' : '' ?>/>&nbsp;Yes
                                </label>
                            </div>
                        </div>
                        <script type="text/javascript">
                            $('#is_video').change(function() {
                                if($('#is_video').prop('checked')) { // checked
                                    $('#video_embed_source').prop('disabled', false);
                                    $('.outer-primary_image').hide('fade');
                                    $('.form-group-video').show('fade');
                                } else {
                                    $('#video_embed_source').prop('disabled', true);
                                    $('.outer-primary_image').show('fade');
                                    $('.form-group-video').hide('fade');
                                }
                            });
                        </script>
                        <div class="form-group form-group-video" <?=(isset($post['is_video']) && !$post['is_video']) || (!isset($post['is_video']))?'style="display:none"':''?>>
                            <label for="video_url">Video URL</label>
                            <input type="text" class="form-control" name="video_url" id="video_url" value="<?= (isset($post['video_url'])) ? $post['video_url'] : '' ?>"/>
                            <label class="mt-3" for="video_embed_source">Video Embed Code (Optional)</label>
                            <textarea rows="3" class="form-control" name="video_embed_source" id="video_embed_source" <?=isset($post['is_video']) && !$post['is_video']?'disabled':''?>><?=(isset($post) && !empty($post['video_embed_source'])) ? $post['video_embed_source'] : '' ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6 outer-primary_image" <?=isset($post['is_video']) && $post['is_video']?'style="display:none"':''?>>
                        <div class="form-group form-group-primary_image">
                            ...
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail fileinput-upload" style="width: 200px; height: 150px;">                        
                                    <?php if (isset($post['primary_image']) && $post['primary_image'] != ''): ?>
                                        ...
                                        <span onclick="delete_picture(this)" class="btn btn-danger btn-delete-photo delete-picture" id="delete-picture" data-id="<?= $post['id_slideshow'] ?>" data-type="primary">x</span>
                                    <?php endif; ?>
                                </div>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            <label for="image_only">Image Only</label>
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" class="flat" value="1" name="image_only" id="image_only" <?= (isset($post['image_only']) && !empty($post['image_only'])) ? 'checked="checked"' : '' ?>/>&nbsp;Yes
                                </label>
                            </div>
                        </div>
                        <div class="form-group">
